                ...
                const std::vector<std::string>& tokens)
{
    ...
    char cmd_char = cmd[0];
    int key = atoi(tokens[0].substr(1, tokens[0].find(",") - 1).c_str());
    int value = atoi(tokens[0].substr(tokens[0].find(",") + 1).c_str());
    
    calc_state_machine::kv_op_type op = calc_state_machine::READ;
    switch (cmd_char) {
    case 'p':   op = calc_state_machine::PUT;   break;
    case 'u':   op = calc_state_machine::UDPATE;   break;
    default:
        op = calc_state_machine::READ;
        key = 0;
        value = 0;
        break;
    };
    ...
    ptr<buffer> new_log = calc_state_machine::enc_log( {op, key, value} );
