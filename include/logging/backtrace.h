...
static SIZE_T_UNUSED _stack_backtrace(void** stack_ptr,
                                      size_t stack_ptr_capacity) {
    return backtrace(stack_ptr, stack_ptr_capacity);
}

static SIZE_T_UNUSED _stack_interpret_linux(void** stack_ptr, char** stack_msg,
                                            int stack_size, char* output_buf,
                                            size_t output_buflen);

static SIZE_T_UNUSED _stack_interpret_apple(void** stack_ptr, char** stack_msg,
                                            int stack_size, char* output_buf,
                                            size_t output_buflen);

static SIZE_T_UNUSED _stack_interpret_other(void** stack_ptr, char** stack_msg,
                                            int stack_size, char* output_buf,
                                            size_t output_buflen);

static SIZE_T_UNUSED _stack_interpret(void** stack_ptr, int stack_size,
                                      char* output_buf, size_t output_buflen) {
    ...
    len = _stack_interpret_linux(stack_ptr, stack_msg, stack_size, output_buf,
                                 output_buflen);

#elif defined(__APPLE__)
    len = _stack_interpret_apple(stack_ptr, stack_msg, stack_size, output_buf,
                                 output_buflen);

#else
    len = _stack_interpret_other(stack_ptr, stack_msg, stack_size, output_buf,
                                 output_buflen);
...
static SIZE_T_UNUSED _stack_interpret_linux(void** stack_ptr, char** stack_msg,
    ...
    for (int i = 1; i < stack_size; ++i) {
        ...
        if (stack_msg[i][fname_len] == '(' &&
            ...
            snprintf(addr_str, 256, "%.*s", upto - fname_len - 2,
        ...
        char cmd[1024];
        snprintf(cmd, 1024, "addr2line -f -e %.*s %s", fname_len, stack_msg[i],
                 addr_str);
        FILE* fp = popen(cmd, "r");
        if (!fp) continue;
        ...
        _snprintf(output_buf, avail_len, cur_len, msg_len,
                  "#%-2zu 0x%016" PRIxPTR " in ", frame_num++, actual_addr);
...
static SIZE_T_UNUSED _stack_interpret_apple(void** stack_ptr, char** stack_msg,
    ...
    for (int i = 1; i < stack_size; ++i) {
        ...
        _snprintf(output_buf, avail_len, cur_len, msg_len, "#%-2zu %s in ",
        ...
        if (filename != exec_file) {
            ...
            _snprintf(output_buf, avail_len, cur_len, msg_len, "%s at %s\n",
                      function_part.c_str(), source_part.c_str());
        }
    }
